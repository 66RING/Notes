...
然而一个移动的节点在子网之间移动时, 并不能维持与远程应用的TCP连接
...
- 双栈法
    - 一个节点具有完整的IPv6和完整的IPv4, 它有发生和接收两者的能力
    - 双栈法中如果任一一个节点是IPv4使能的, 则必须使用IPv4
        - 因为当IPv6节点要向一个IPv4节点发送数据报时, 需要"裁剪"自身的报文一符合IPv4
        - 而"裁剪"后就没办法恢复了
- 隧道法(双栈法法的一种)
...
- 分布式
    - 每个节点都要从一个或多个直接相连邻居接收某些信息，然后将计算结果分发给邻居
- 迭代的
...
$c(x, v)$表示邻居节点v的距离， $d_v(y)$表示v到y的最低费用
...
在DV算法中，每当节点x发现他的直接相连的链路费用发生变化或收到邻居发来的更新时，
它就更新(如果有更低费用路径的话)其转发表对应位置的距离向量。
同理，每个节点执行同x的操作，转发表更新时通知相邻节点。
节点重新计算距离向量后，它再次发送他们更新后的结果给邻居节点。
当所有节点已得到最到路径无需更新时，没有互相发生信息通知，从而进入等待状态。
...
在**广播路由选择**中，网络提供了从一种源节点到网络中所有其他节点交付分组的服务;
**多播路由选择**使单个源节点能向其他网络节点的子集发送分组的副本。


#### 广播路由选择算法

- N次单播
    - 源节点产生该分组的N个副本，对不同目的地的每个副本进行编址并传输
    - 缺点
        - 效率底
        - 要获取所以节点的信息就又要获取全局的数据

显然更有效率的方式是，经第一跳仅发送分组的单个副本，
然后让第一跳后面其他端的节点生成并转发任何附加的所需副本。
类似细胞分裂。

- 无控制洪泛(flooding)
    - 该方法要求源节点向它所有邻居节点发送分组副本。
    - 当某节点收到一个广播的分组时重复上一步操作
    - 缺点
        - 如果图中有圈，广播将会无限循环
        - 产生大量重复的分组副本，导致**广播风暴**
- 受控洪泛
    - **序号控制洪泛**
        - 源节点将其地址以及**广播序号**放入广播分组，再向邻居发送分组
        - 每个节点维护它已经收到的，复制和转发的源地址和每个广播分组的序号列表
    - **反向路径转发(RPF)**
...
- 生成树广播
    ...
    - 接收广播分组的节点则向生成树的所以邻居转发该分组
    - 注意到一个节点不必知道整棵树，只需要知道它在的哪些邻居是生成树中的
...
- 任何主机都能加入一个组吗？谁来限制？
- 一个组成员知道其他组成员的标识吗？
- 网络节点互相之间如何交互，以向所以组成员交付一个多播数据报呢？
